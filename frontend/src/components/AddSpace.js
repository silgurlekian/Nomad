...
const AddSpace = () => {
    const [formData, setFormData] = useState({
        ...
        precio: "",
        selectedServices: [],
        selectedSpacesType: "", // Cambiado a cadena para un solo valor
    ...
    useEffect(() => {
        ...
        const fetchSpacesType = async () => {
            try {
                const response = await axios.get("http://localhost:3000/api/spacesType");
                setAllSpacesType(response.data);
            } catch (error) {
                setGlobalError("Error al cargar los tipos de espacios: " + error.message);
            }
        };
    ...
    const validateForm = () => {
        const { nombre, direccion, ciudad, precio, selectedServices, selectedSpacesType } = formData;
        const newErrors = {};
        ...
        if (!precio || precio <= 0) newErrors.precio = "El precio debe ser mayor que cero.";
        if (selectedServices.length === 0) newErrors.selectedServices = "Los servicios son obligatorios.";
        if (!selectedSpacesType) newErrors.selectedSpacesType = "Los tipos de espacio son obligatorios."; // Cambiado a validación de cadena
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            Object.entries(formData).forEach(([key, value]) => {
                ...
                } else if (key === "selectedSpacesType") { // Ahora solo se espera un valor
            ...
            <form onSubmit={handleSubmit}>
                <div className="mb-3">
                    ...
                    <select 
                        ...
                        className={`form-control ${errors.selectedSpacesType ? "is-invalid" : ""}`}
                    >
                        ...
                        {allServices.length > 0 ? (
                            allServices.map((service) => (
                                <div key={service._id} className="form-check">
                                    ...
                                    />
                                    <label htmlFor={`service-${service._id}`} className="form-check-label">
                                        {service.name}
                                    </label>
                                </div>
                            ))
                           ...
                           {["hora", "dia", "mes", "anual"].map((tipo) => (
                               <div key={tipo} className='form-check'>
                                   <input
                                       ...
                                       id={`reserva-${tipo}`}
                                       name={tipo}
                                       checked={!!formData.tiposReservas[tipo]} // Asegúrate de que sea booleano
                                       onChange={(e) => {
                                       ...
                                       className='form-check-input'
                                   />
                                   <label htmlFor={`reserva-${tipo}`} className='form-check-label'>
                                       Por {tipo}
                                   </label>
                               </div>
                           ))}
               ...
               <div className='mb-3'>
                   <label htmlFor='imagen' className='form-label'> Imagen del espacio </label>
                   <input
                       type='file'
                       id='imagen'
                       name='imagen'
               ...
               <button type='submit' className='btn btn-primary'>Agregar Espacio</button>
           </form>
       </div>
   );
};
